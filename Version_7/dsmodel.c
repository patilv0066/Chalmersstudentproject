/* DSblock model generated by Dymola from Modelica model Version_7.Vehicles.vehicle
 Dymola Version 2017 (64-bit), 2016-06-23 translated this at Sun Jan 01 11:27:00 2017
...
#define NHash1_ -1549743647
#define NHash2_ 888222279
#define NHash3_ 1264080739
#define NI_    0
#define NRelF_ 7
#define NRel_  7
#define NTim_  0
...
DYMOLA_STATIC const char*GUIDString="{b7f62e51-0ca3-4b3c-a1a3-6bbed2d046e3}";
...
DYNX(W_,9) = IF Greater(DYNX(F_,0),"der(trans.Vx)", DYNX(DP_,8),"trans.axh", 2)
  ...
  DYNX(F_,0)-DYNX(DP_,9))*(DYNX(DP_,10)-1),"(der(trans.Vx)-trans.axl)*(trans.E_factor-1)",
  DYNX(DP_,8)-DYNX(DP_,9),"trans.axh-trans.axl"));
DYNX(DYNhelp,78) = DYNX(W_,15)-209.43951023931953;
DYNX(DYNhelp,79) = tanh(0.01*DYNX(DYNhelp,78));
DYNX(W_,6) = DYNX(W_,5)*DYNX(W_,9)*(1-DYNX(DYNhelp,79))*DYNX(W_,4)-40;
DYNX(W_,11) = RealBmin(DYNX(W_,6), DYNX(DP_,11));
DYNX(W_,13) = DYNX(W_,11)+DYNX(X_,1);
DYNX(W_,7) = IF DYNX(X_,0) < 0 THEN RealBmax(0, DYNX(W_,13)) ELSE DYNX(W_,13);
...
NonLinearSystemOfEquationsNH(Jacobian__, residue__, x__, 1, 0, 2, 9, 10, 
  DYNX(DYNhelp,80), 55, DYNX(did_->helpvari_vec,23), 23);
...
Residues;
  DYNX(W_,35) = 0.5*(DYNX(F_,7)+DYNX(F_,8));
  DYNX(W_,33) = DYNX(W_,35)*DYNX(DP_,60);
  DYNX(DYNhelp,135) = RealElement( RealTemporaryDense( &DYNX(DP_,48), 1, 12), 
    (SizeType)(DYNX(W_,25)));
  PopAllMarks();
  DYNX(W_,31) = DYNX(W_,33)*DYNX(DYNhelp,135);
  DYNX(DYNhelp,136) = 209.43951023931953 < DYNX(DYNhelp,77);
  DYNX(W_,16) = IF DYNX(DYNhelp,136) THEN 0.0 ELSE IF DYNX(W_,30) > 
    62.83185307179586 THEN DYNX(W_,31) ELSE 0.0;
  DYNX(W_,27) = DYNX(W_,7)-DYNX(DP_,106)*DYNX(W_,16);
  DYNX(W_,28) = DYNX(W_,27)*DYNX(DYNhelp,135);
  DYNX(W_,29) = DYNX(DP_,61)*DYNX(W_,28)*DYNX(DP_,60);
  DYNX(W_,37) = RealBmax(DYNX(W_,29), 0);
  DYNX(DYNhelp,137) = 1*DYNX(W_,39);
  DYNX(DYNhelp,138) = RealBmax(DYNX(W_,27), 0);
  DYNX(W_,38) =  -DYNX(DYNhelp,137)*DYNX(DYNhelp,138)*DYNX(DP_,53)*DYNX(DP_,60);
  DYNX(DYNhelp,139) = Equal(DYNX(U_,4), 1, 4);
  DYNX(W_,70) = IF DYNX(DYNhelp,139) THEN DYNX(W_,38) ELSE DYNX(W_,37);
DYNX(DYNhelp,140) = tanh(10*DYNX(X_,8));
SetVector(residue__, 1, DYNX(DP_,116)*DYNX(F_,8)-DYNX(W_,70)+DYNX(DYNhelp,140)*
  DYNX(DYNhelp,70)*DYNX(W_,50)+DYNX(Y_,1)*DYNX(DP_,112));

Jacobian(Jacobian__)
MatrixZeros(Jacobian__);
DYNX(DYNhelp,141) = DYNX(W_,27) > 0;
DYNX(DYNhelp,142) = DYNX(DP_,106)*(DYNX(DYNhelp,135)*DYNX(DP_,60)*DYNX(DP_,61)*(
  IF DYNX(W_,29) > 0 THEN 1.0 ELSE 0.0)*(IF DYNX(DYNhelp,139) THEN 0.0 ELSE 1.0)
  -DYNX(DP_,60)*DYNX(DP_,53)*DYNX(DYNhelp,137)*(IF DYNX(DYNhelp,141) THEN 1.0
   ELSE 0.0)*(IF DYNX(DYNhelp,139) THEN 1.0 ELSE 0.0));
SetMatrixLeading(Jacobian__, 1, 1, 1, DYNX(DP_,116)+0.5*DYNX(DP_,60)*
  DYNX(DYNhelp,135)*(IF DYNX(DYNhelp,136) THEN 0.0 ELSE IF DYNX(W_,30) > 
  62.83185307179586 THEN 1.0 ELSE 0.0)*DYNX(DYNhelp,142));
...
DYNX(F_,2) = DYNX(Y_,9)-DYNX(X_,0)*DYNX(X_,4);
DYNX(F_,3) = DYNX(X_,4);
DYNX(DYNhelp,143) = cos(DYNX(X_,3));
DYNX(DYNhelp,144) = sin(DYNX(X_,3));
DYNX(F_,5) = DYNX(X_,0)*DYNX(DYNhelp,143)-DYNX(X_,2)*DYNX(DYNhelp,144);
DYNX(F_,6) = DYNX(X_,2)*DYNX(DYNhelp,143)+DYNX(X_,0)*DYNX(DYNhelp,144);
